...
from workouts.serializers import WorkoutOnlySerializer  
    ...
    def get(self, request, *args, **kwargs):
        ...
        workouts = Workout.objects.filter(
            ...
            & (
                ...
                | Q(calories_burn__icontains=query)
                | Q(duration_minutes__icontains=query)
                | Q(rounds__name__icontains=query)
                | Q(rounds__exercises__name__icontains=query)
                | Q(rounds__exercises__tips__icontains=query)
            )
        ).distinct()
        meals = Meal.objects.filter(user=request.user, title__icontains=query)
        workout_serializer = WorkoutOnlySerializer(workouts, many=True)
